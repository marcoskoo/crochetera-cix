...
export async function POST(req: NextRequest) {
  ...
  if (file.size > maxSize) {
    return NextResponse.json({ error: `El archivo supera el máximo permitido (${isVideo ? '50MB' : '10MB'})` }, { status: 413 })
  }
  ...
  if (hasBlobToken) {
    try {
      ...
      if (isImage) {
        try {
          finalBuffer = await sharp(buffer).resize(1600, 1600, { fit: 'inside', withoutEnlargement: true }).jpeg({ quality: 85 }).toBuffer()
          contentType = 'image/jpeg'
        } catch {}
      }
      const blob = await put(fileName, finalBuffer, { access: 'public', contentType, addRandomSuffix: false })
      return NextResponse.json({ url: blob.url, type: isVideo ? 'video' : 'image' })
    } catch (e) {
      console.error('Error subiendo a Vercel Blob:', e)
      return NextResponse.json({ error: 'Error al subir archivo a Blob Storage' }, { status: 500 })
    }
  }
  ...
  try {
    ...
    if (isImage) {
      ...
      try {
        await sharp(buffer).resize(1600, 1600, { fit: 'inside', withoutEnlargement: true }).jpeg({ quality: 85 }).toFile(filePath)
      } catch {
